...
We compare unconstrained RRT planning against corridor-constrained planning (Section~\ref{sec:planning}) using 1{,}000 randomly sampled goals at heights $z \in [1.0, 1.47]$\,m. Table~\ref{tab:rrt-corridor} summarizes the results.

\input{tab_rrt_corridor}

The two strategies represent a fundamental tradeoff between plan quality and planning speed. RRT is unconstrained: the sampling-based planner freely explores joint space to find a collision-free path, and does so very quickly (avg.\ 0.21\,s). The resulting paths are valid but indirect, averaging 1.67\,m of joint-space travel for goals separated by only $\sim$0.99\,m in Cartesian space (path efficiency 0.750). The corridor constraint forces the end-effector to travel inside a tight box aligned with the start-to-goal segment, producing geometrically straight, predictable motions (avg.\ 1.23\,m path length, efficiency 0.889). The cost is a 200$\times$ increase in planning time (41.41\,s on average, median 20.55\,s) as the constrained sampler must find a narrow feasible corridor, and a drop in success rate from 99.0\,\% to 90.4\,\% as some goals cannot be reached within the box. For a collaborative workspace where human workers are nearby, the corridor's straight, legible motions justify this cost.

Figure~\ref{fig:plantime-hist} shows the plan time distributions on a logarithmic scale. RRT plan times cluster tightly around the 0.10\,s median, while corridor-constrained planning spans several decades with a median of 20.55\,s and a long tail of near-timeout plans.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig_rrt_corridor_hist.png}
\caption{Plan time distributions for unconstrained RRT (blue) vs.\ corridor-constrained planning (orange). Dashed lines indicate the median for each strategy.}
\label{fig:plantime-hist}
\end{figure}

Figure~\ref{fig:plantime-scatter} shows a per-goal comparison of plan times. Nearly all points lie above the diagonal, confirming that the corridor constraint increases planning time at every goal. Blue points mark goals where only RRT succeeded, concentrated at the long-plan-time edge where the constrained planner timed out.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig_plantime_scatter.png}
\caption{Per-goal plan time comparison. Each point represents one of 1{,}000 goals. Points above the diagonal indicate corridor planning was slower than RRT.}
\label{fig:plantime-scatter}
\end{figure}

Figure~\ref{fig:spatial-failures} shows the spatial distribution of failures. RRT failures (10 total) are scattered across the workspace with no clear spatial pattern, while corridor failures (96 total) cluster at the positive-$x$ boundary, where the corridor box extends into kinematically unreachable regions.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{fig_spatial_failures.png}
\caption{Spatial distribution of planning failures. Left: unconstrained RRT (10 failures, scattered). Right: corridor-constrained planning (96 failures), concentrated at the positive-$x$ workspace boundary.}
