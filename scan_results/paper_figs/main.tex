...
Standard iterative solvers such as KDL, TracIK, and PICK-IK return a single,
seed-dependent solution per call when applied to a redundant system.
For a 7-DoF manipulator this is insufficient: a single call cannot enumerate
the qualitatively different configurations that arise from varying the gantry
position or selecting a different elbow/shoulder family, leading to low
planning success rates (Table~\ref{tab:ik_comparison}).
Extending to multiple random restarts improves coverage but incurs significant
per-call overhead without guaranteeing family coverage.
We instead implement a closed-form solver that exploits the UR16e's spherical
wrist to decouple position and orientation subproblems and explicitly
enumerates all kinematic families.

The 7-DoF system admits up to eight analytical arm solutions per fixed gantry
position: two shoulder-pan solutions from the horizontal wrist-centre position,
two wrist-2 solutions from the $z$-axis rotation constraint, and two elbow
configurations from the resulting planar 2R inverse kinematics.
The solver is evaluated at up to 21 candidate gantry positions offset from the
current position in 0.1\,m increments across $\pm1.0$\,m, clamped to the
actuator travel limits, yielding up to 168 candidates per target pose.
Solutions are filtered by joint limits, a minimum end-effector clearance of
0.65\,m from the gantry plate, and a triangle-area validity check on the
wrist--forearm--gantry triangle to reject near-singular configurations.

\begin{table}[!t]
\caption{IK solver comparison over 1\,000 random goal poses with a 20\,s
planning budget. \textbf{Bold} = best.}
\label{tab:ik_comparison}
\centering
\begin{tabular}{l c c c}
\toprule
IK Method & Candidates & SR (\%)$\uparrow$ & Avg.\ Plan Time (s)$\downarrow$ \\
\midrule
KDL, 1 seed            & 1         & 25.7          & 1.47  \\
KDL, 50 random seeds   & $\leq$50  & 73.0          & 10.46 \\
Analytical (ours)      & $\leq$168 & \textbf{99.3} & \textbf{0.31} \\
\bottomrule
\end{tabular}
\end{table}
...
We evaluate the effect of the three IK seed selection cost components (joint distance, proximity penalty, area penalty) using eight weight configurations that span the space of the three costs: baseline (all weights zero), all equal, each cost ablated in turn, and each cost in isolation. Goals are sampled with varying heights ($z \in [1.0, 1.47]$\,m) and planned from a fixed home position. Table~\ref{tab:cost-ablation} summarizes the results.

\input{tab_cost_ablation}

The baseline configuration ($\alpha = \beta = \gamma = 0$, uniform random IK seed selection) achieves the highest success rate (99.6\%) but the slowest average plan time (0.96\,s) and longest paths (2.40\,m). The proximity-only configuration achieves the highest manipulability (0.700), as configurations that maintain clearance from the gantry tend to be more dexterous. We adopt the no-area configuration ($\alpha = \beta = 1$, $\gamma = 0$) as our final choice: it ranks second on both success rate and manipulability among the eight cases, trading a small drop in success and manipulability for faster plan times and shorter paths than proximity-only and baseline.

Figure~\ref{fig:manip-bar} summarizes the mean manipulability score across all eight configurations. The proximity term is the dominant driver of dexterity: every configuration that includes it (Prox, \texttildelow Area, \texttildelow Joint, Equal) clusters near $\mu \approx 0.63$--$0.70$, while configurations that omit it (Base, Area, Joint, \texttildelow Prox) fall below $\mu \leq 0.49$. Notably, the area-only and no-proximity configurations achieve the two lowest manipulability scores (0.421 and 0.424 respectively), confirming that the triangle-area penalty alone is insufficient as a dexterity surrogate. The large within-group standard deviations reflect workspace heterogeneity: goals near the kinematic workspace boundary force less dexterous configurations regardless of the cost weights.

\begin{figure}[!t]
\centering
\includegraphics[width=\columnwidth]{fig_manip_bar.png}
\caption{Mean manipulability score ($\pm$1\,std.) for each of the eight cost configurations. Configurations that include the proximity term (Prox, \texttildelow Area, \texttildelow Joint, Equal) consistently achieve higher dexterity. Best configuration highlighted in orange.}
\label{fig:manip-bar}
\end{figure}

To confirm that these differences are statistically significant rather than artefacts of sampling variability, we applied a Kruskal--Wallis test~\cite{kruskal1952use}---a non-parametric one-way analysis of variance on ranks---across all eight configurations. The test confirmed highly significant variation in manipulability score ($H(7)=2523.7$, $p<0.001$), plan time ($H(7)=1055.7$, $p<0.001$), and path length ($H(7)=448.7$, $p<0.001$). To isolate the contribution of the proximity term specifically, we performed pairwise Mann--Whitney U tests~\cite{mann1947test} between each of the four proximity-inclusive configurations (Prox, \texttildelow{}Area, \texttildelow{}Joint, Equal) and each of the four proximity-exclusive configurations (Base, Area, Joint, \texttildelow{}Prox), yielding 16 comparisons. After Bonferroni correction~\cite{dunn1961multiple}, all 16 pairs remain significant ($p<0.001$), confirming that the proximity term is the dominant and statistically reliable driver of dexterity in the IK seed selection cost function.

\subsubsection{Path Safety: Unconstrained vs.\ Corridor-Constrained Planning}\label{sec:corridor-results}
Using the no-area cost configuration selected above, we evaluate the tradeoff between
planning speed and path predictability on the same 1\,000 goal poses
(Table~\ref{tab:rrt-corridor}).

\input{tab_rrt_corridor}

Unconstrained RRT freely explores joint space to find any collision-free path.
This is fast (avg.\ 0.51\,s) and nearly always succeeds (99.4\%), but the
resulting trajectories are indirect: the sampler may swing the arm through
unpredictable arcs that, while collision-free with the static environment model,
can bring the end-effector close to battery components on the table or pass
through regions that are difficult for a nearby human worker to anticipate.

The corridor constraint addresses this by requiring the end-effector to remain
inside a tight box aligned with the start-to-goal direction vector throughout
the motion.
This produces geometrically straight, legible paths that are 29\% shorter in
end-effector travel (1.26\,m vs.\ 1.78\,m) and avoid lateral excursions over
the work surface.
The cost is a ${\sim}93{\times}$ increase in average planning time (47.4\,s
vs.\ 0.51\,s), as the constrained sampler must find a feasible path within a
narrow spatial tube, and a 6\,pp drop in success rate (93.4\% vs.\ 99.4\%) as
some goals cannot be reached while keeping the end-effector inside the box.
For an assembly disassembly scenario where human workers share the workspace and
battery components occupy the table surface, the corridor's predictable,
straight motions justify this planning overhead.
...
\subsection{Planning}
The dominant factor in planning success is exhaustive enumeration of kinematic
families rather than iterative seed restarts. This is consistent with findings
in redundant manipulator literature where seed diversity, not restart
count, determines coverage of the solution space.

The corridor constraint produces predictable, straight end-effector motions at
the cost of longer planning times. For a fixed battery model this overhead is
amortized by offline precomputation of the TSP weight matrix, making it
practical in a production setting. More importantly, legible robot motion is a
safety requirement in a shared workspace, where a co-worker must
anticipate the robot's path rather than react to it.
\subsection{Vision}
\subsection{Agentic AI}
